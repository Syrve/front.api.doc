...
In the `EditorTester` window included in the SamplePlugin, where examples of order editing are collected, a checkbox has been added “Apply changes to order that is opened in Syrve POS”; when enabled, the changes are applied to the currently open order on the screen.